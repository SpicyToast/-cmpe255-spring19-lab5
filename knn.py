...
def predict_preferred_language_by_city(k_values, cities):
    ...
    for k in k_values:
        ...
        for location, language_act in cities:
            others = [others 
                        for others in cities
                        if others != (location, language_act)]
            language_pred = knn_classify(k, others, location)

            if language_pred == language_act:
                correct_pred +=1 

        print (k, "neighbor[s]:", correct_pred, "correct out of", len(cities))
